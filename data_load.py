...
def load_ch_vocab():
    vocab = []
    with codecs.open('preprocessed/ch.vocab.tsv', 'rb', 'utf-8') as f:
...
def load_en_vocab():
    vocab = []
    with codecs.open('preprocessed/en.vocab.tsv', 'r', 'utf-8') as f:
        for line in f.readlines():
            if int(line.split("\t")[1]) >= hp.min_cnt:
                vocab.append(line.split("\t")[0])
...
def create_data(source_sents, target_sents):
    ...
    for source_sent, target_sent in zip(source_sents, target_sents):
        ...
        y = [ch2idx.get(word, 1) for word in jieba.lcut(target_sent)[:-1] + ["</S>"]]
...
def create_testA_data(source_sents):
    en2idx, idx2en = load_en_vocab()
    print("test dict done")
    x_list, Sources = [], []
    append_len = 0
    append_list = []
    for index, source_sent in enumerate(source_sents):
        x = [en2idx.get(word, 1) for word in nltk.word_tokenize(source_sent) + ["</S>"]]  # 1: OOV, </S>: End of Text
        if len(x) <= hp.maxlen:
            x_list.append(np.array(x))
        else:
            x_list.append(np.array(x[:hp.maxlen - 1] + [3]))
            # num = len(x) // (hp.maxlen - 1)
            # for i1 in range(num):
            #     x.insert((i1 + 1) * 4 - 1, 3)
            # for i2 in range(num):
            #     x_list.append(x[i2 * 4:(i2 + 1) * 4])
            # if x[-1] != 3:
            #     x.append(3)
            #     x_list.append(x[num * 4:])
            #     num += 1
            # append_list.append([index+append_len, num])
            # append_len += num
    ...
    for each_seg in train_seg:
        with codecs.open(each_seg, 'r', 'utf-8') as f:
            ...
            for line in f.readlines():
                ...
                ch_sents.append(line.split("\t")[3].rstrip("\n"))
                i += 1
                if i%100000==0:
                    print(i)
            print("one seg done")
...
def load_test_data():
    ...
    for root, dirs, files in os.walk(hp.source_target_train_dir):
        for file in files:
            test_seg.append(os.path.join(root, file))
    en_sents = []
    for each_seg in test_seg:
        with codecs.open(each_seg, 'r', 'utf-8') as f:
            for line in f.readlines():
                en_sents.append(line.split("\t")[2].rstrip("\n"))
    ...
    X, Sources = create_test_data(en_sents)
...
def load_testA_data():
    ...
    en_sents = [_re_enfine(line) for line in codecs.open(hp.source_test, 'r', 'utf-8').read().split("\n") if
                line and line[:4] == "<seg"]
    # max = 0
    # for i in range(len(en_sents)):
    #     if max<len(en_sents[i].split()):
    #         k = i
    #         max = len(en_sents[i].split())
    # print(k)
    # print(max)
    # print(en_sents[k])
    X = create_testA_data(en_sents)
    return X  # (1064, 150)
...
def load_valid_data():
    ...
    # _refine每次处理一行
    en_sents = [_re_enfine(line) for line in codecs.open(hp.source_valid, 'r', 'utf-8').read().split("\n") if
                line and line[:4] == "<seg"]
    ch_sents = [_re_chfine(line) for line in codecs.open(hp.target_valid, 'r', 'utf-8').read().split("\n") if
                line and line[:4] == "<seg"]
